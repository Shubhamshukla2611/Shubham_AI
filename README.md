...
The repo ships with a `render.yaml` Blueprint and a `backend/Dockerfile` so the entire backend can be deployed to Render in a few minutes.

### 1. Push to GitHub
```bash
git add .
git commit -m "Render-ready: Dockerfile, render.yaml, auto-ingest"
git push origin main
```

### 2. Create the Blueprint on Render
1. Open https://dashboard.render.com → **New +** → **Blueprint**.
2. Connect your GitHub repo. Render will read `render.yaml` and create:
   - A **Web Service** (`shubham-ai-backend`) running the Dockerfile in `backend/`.
   - A **1 GB persistent disk** mounted at `/app/backend/data` (Chroma index + HuggingFace cache).

### 3. Set the secrets
In the Render service → **Environment**, fill in:
| Key | Where to get it |
|-----|-----------------|
| `GROQ_API_KEY` | https://console.groq.com/keys |
| `VAPI_API_KEY` | https://dashboard.vapi.ai |
| `TWILIO_AUTH_TOKEN` | Twilio console (Vapi uses it) |
| `GOOGLE_CALENDAR_CREDENTIALS_JSON` | Paste your service-account JSON (single line) |

`VOICE_WEBHOOK_SECRET` is auto-generated by `render.yaml`. The rest of the env (`ALLOWED_ORIGINS`, `GROQ_MODEL`, `MIN_SCORE`, etc.) ships with sensible defaults — adjust if needed.

### 4. First boot
On the very first boot, `/app/backend/data` is empty. The app's lifespan hook detects this and runs the in-process ingest pipeline against the source documents that ship with the repo (under `backend/data/raw/`). The Chroma index is then written to the persistent disk, so subsequent deploys just load it.

Watch the **Logs** tab for:
```
Auto-ingest: building Chroma index from /app/backend/data/raw
Auto-ingest complete | chunks=N
RAG pipeline initialized after auto-ingest
```

### 5. Verify
- Health: `https://<your-service>.onrender.com/health`
- Docs: `https://<your-service>.onrender.com/docs`
- Smoke test: `curl -X POST https://<your-service>.onrender.com/api/chat -H "Content-Type: application/json" -d '{"message":"Tell me about yourself"}'`

### 6. Connect the frontend
Update the Vercel project's env (or whichever host serves the React app):
```
VITE_API_URL=https://<your-service>.onrender.com
```
Redeploy the frontend. CORS is already configured in `render.yaml` for `https://shubham-ai-omega.vercel.app`.

### Cost notes
- Free/Starter plan (512 MB RAM) is enough for chat traffic. Bump to **Standard** (2 GB) for steady voice-agent traffic, since the Vapi webhook tooling loads more dependencies.
- The persistent disk is **$1/month for 1 GB** — the Chroma index for this KB is ~2 MB, and the HuggingFace MiniLM cache is ~80 MB. A 1 GB disk is plenty.
